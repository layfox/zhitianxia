			...
			<movable-area id="dragSortArea" class="dragSort-area" :style="{height: boxHeight + 'px'}">
				<block v-for="(item,index) in cloneList" :key="item.id">
					<movable-view class="dragSort-view" direction="all"
						...
						<view class="dragSort-view__con" :style="{background: item.disabled ? '#F2F2F2' : '', color: item.disabled ? '#999' : ''}">
							<view class="dragSort-view__label is-ellipsis">{{item[label]}}</view>
							<text class="dragSort-view__btn-del" v-if="isEdit&&!item.disabled" @click="handleDel(item)"></text>
						</view>
					</movable-view>
				</block>
			</movable-area>
...
<style scoped>
	...
	.dragSort-view__btn-del { position: absolute; width: 20rpx; height: 20rpx; right: 8rpx; top: 8rpx; background: url('~@/static/channel-close.png') no-repeat; background-size: 20rpx 20rpx;}
</style>
